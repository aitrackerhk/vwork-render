...
func main() {
	...
	// 4. Fetch all products and determine the safe starting sequence number
	fmt.Println("=> Step 4: Scanning all existing product codes to find a safe starting number...")
	getProductsReq, _ := http.NewRequest("GET", baseURL+"/api/v1/products?limit=5000", nil) // High limit to get all products
	...
	if err := json.NewDecoder(getProductsResp.Body).Decode(&productsData); err != nil {
		log.Fatalf("Failed to decode product data: %v", err)
	}

	productsToFix := []Product{}
	maxSeq := 0
	todayStr := time.Now().Format("20060102")
	prefix := "PROD-" + todayStr + "-"

	for _, p := range productsData.Data {
		if p.Code == "" || strings.HasPrefix(p.Code, "P-") {
			productsToFix = append(productsToFix, p)
		} else if strings.HasPrefix(p.Code, prefix) {
			seqStr := strings.TrimPrefix(p.Code, prefix)
	...
	fmt.Printf("=> Scan complete. Found %d products to fix.\n", len(productsToFix))
	fmt.Printf("=> Today's prefix is '%s'. The highest sequence number used today is %d.\n", prefix, maxSeq)
	fmt.Printf("=> New codes will start from %d.\n", maxSeq+1)

	// 5. Update products with correctly formatted and safe codes
	if len(productsToFix) == 0 {
		fmt.Println("=> No products needed fixing. Done.")
		return
	}

	fmt.Println("=> Step 5: Updating products with safe, correctly formatted codes...")
	updateCount := 0
	for i, p := range productsToFix {
		newSequence := maxSeq + i + 1
		newCode := fmt.Sprintf("%s%03d", prefix, newSequence) // Assuming 3-digit padding
		...
		if updateResp.StatusCode == http.StatusOK {
			updateCount++
			fmt.Printf("  - SUCCESS: Updated product %s (%s) with new code %s\n", p.Name, p.ID, newCode)
		} else {
		...
		time.Sleep(200 * time.Millisecond) // Be respectful to the server
	}
	...
	verifyReq, _ := http.NewRequest("GET", baseURL+"/api/v1/products?limit=5000", nil)
	...
	for _, p := range verifyData.Data {
		for _, fixedP := range productsToFix {
			if p.ID == fixedP.ID && p.Code == "" {
				remainingIssues++
	...
	if remainingIssues == 0 {
		fmt.Println("=> VERIFICATION PASSED: All previously identified products now have a code.")
	} else {
